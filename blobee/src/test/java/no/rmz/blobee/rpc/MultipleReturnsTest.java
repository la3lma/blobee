...
public final class MultipleReturnsTest {
    ...
    public final class ServiceTestItem extends Testservice.RpcService {
        ...
        public void invoke(
            ...
            if (controller instanceof BlobeeRpcController) {
                final BlobeeRpcController bc = (BlobeeRpcController) controller;
                final boolean isMulti = bc.isMultiReturn();
                org.junit.Assert.assertTrue(isMulti);
            } else {
                fail("Not a blobee controller");
            }
    ...
    // XXX IMPORTANT!  Add a test for ordinary methods that will _BREAK_
    //                 if multiple returns are attempted without having
    //                 the controller declaring it kosher.
    //                 Also, we need some scheme to get rid of the
    //                 garbage generated by multi-returning methods that
    //                 die, become stale, ends etc.
}
